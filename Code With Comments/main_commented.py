            ...
            if game.valid_guess(u_inp) == True:
                ...
                if 'P' in str(control):
                    ...
                    for c,b in zip(game.colors,game.board):
                        ...
                        guess_string="".join(b)
                        if guess_string != colors_string:  #if both are equal then its an empty line meaning game has not been played yet
...
#Work in progress, still trying to fix some bugs
    ...
    for i in range(ROWS):
        ...
        game.colors[i] = [s for s in str(u_inp)]
